 ...
 * - 板块支持 type：text / daily / checklist，新建小结为空画布（无默认模板）
 */
const fs = require("fs");
...
function defaultSections() {
  // 新版：空画布，由用户自由添加文本块 / 每日完成情况 / 清单 / 总结模板
  return [];
}
...
function load() {
  ...
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      for (const s of cache) normalize(s);
...
function create(data, spaceId) {
  ...
  const summary = {
    ...
    sections: (data && Array.isArray(data.sections) && data.sections.length ? data.sections : defaultSections()).map((x) => ({
      id: x.id || newId(),
      title: x.title || "",
      content: x.content || "",
      type: x.type || "text",
...
function update(id, data, spaceId) {
  ...
  if (legacy) {
    ...
    for (const sec of incoming) {
      ...
      if (ex) {
        if (ex.title !== (sec.title || "") || ex.content !== (sec.content || "") || (ex.type || "text") !== (sec.type || "text")) {
          ex.title = sec.title || "";
          ex.content = sec.content || "";
          ex.type = sec.type || "text";
          ex.updatedAt = now;
          changed = true;
        }
      } else {
...
module.exports = { reload, list, get, create, update, remove, addImage, removeImageFromAll, imageInSpace, assignSpaceId, defaultSections };
