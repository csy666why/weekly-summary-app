...
function defaultSections() {
  const now = nowISO();
  return [
    { id: "s-" + Date.now() + "-1", title: "（一）工作内容", content: "", updatedAt: now },
    { id: "s-" + Date.now() + "-2", title: "（二）收获与体会", content: "", updatedAt: now },
    { id: "s-" + Date.now() + "-3", title: "（三）下周计划", content: "", updatedAt: now }
  ];
}
...
function load() {
  ...
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      for (const s of cache) if (!Array.isArray(s.images)) s.images = [];
    } else if (fs.existsSync(SEED_FILE)) {
      cache = JSON.parse(fs.readFileSync(SEED_FILE, "utf8"));
      if (!Array.isArray(cache)) cache = [];
      for (const s of cache) {
        if (!Array.isArray(s.images)) s.images = [];
        for (const sec of s.sections || []) if (!sec.updatedAt) sec.updatedAt = s.updatedAt || nowISO();
      }
      persistNow();
    } else {
      cache = [];
...
function create(data, spaceId) {
  ...
  const summary = {
    ...
    sections: (data && Array.isArray(data.sections) && data.sections.length ? data.sections : defaultSections()).map((x) => ({ id: x.id || newId(), title: x.title || "", content: x.content || "", updatedAt: now })),
...
function update(id, data, spaceId) {
  ...
  if (legacy) {
    const merged = incoming.map((x) => ({ id: x.id || newId(), title: x.title || "", content: x.content || "", updatedAt: now }));
    ...
    for (const sec of incoming) {
      ...
      if (ex) {
        if (ex.title !== (sec.title || "") || ex.content !== (sec.content || "")) {
          ex.title = sec.title || "";
          ex.content = sec.content || "";
          ex.updatedAt = now;
          changed = true;
        }
      } else {
        s.sections.push({ id: sec.id, title: sec.title || "", content: sec.content || "", updatedAt: now });
        changed = true;
      }
    }
    if (edited.reorder) {
