"use strict";
/* 极简 ZIP：写入用 store；读取支持 store 与 deflate（用于 docx/pptx 解析） */
...
function readZip(buf) {
  ...
  const cenStart = buf.readUInt32LE(eocd + 16);
  const files = [];
  let pos = cenStart;
  for (let i = 0; i < count; i++) {
    ...
    let data;
    if (method === 0) data = buf.subarray(dataStart, dataStart + compSize);
    else if (method === 8) data = zlib.inflateRawSync(buf.subarray(dataStart, dataStart + compSize));
    else throw new Error("仅支持 store/deflate 压缩（method " + method + "）");
