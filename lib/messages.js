...
function add({ from, to, type, content, imageId, emoji }) {
  load();
  const t = type === "image" ? "image" : type === "emoji" ? "emoji" : "text";
  const m = {
    id: newId(),
    from: String(from),
    to: String(to),
    type: t,
    content: t === "image" || t === "emoji" ? "" : String(content || ""),
    imageId: t === "image" ? imageId || "" : "",
    emoji: t === "emoji" ? String(emoji || "") : "",
...
function remove(id, spaceId) {
  ...
  cache.messages = cache.messages.filter((x) => !(x.id === id && (x.from === spaceId || x.to === spaceId)));
  if (cache.messages.length !== before) { persist(); return true; }
  return false;
}
function recall(id, spaceId) {
  load();
  const x = cache.messages.find((it) => it.id === id && it.from === spaceId);
  if (!x) return null;
  if (Date.now() - new Date(x.createdAt).getTime() > 120000) return { tooOld: true };
  x.type = "text";
  x.content = "（已撤回）";
  x.recalled = true;
  persist();
...
module.exports = { reload, list, listAll, add, markRead, unreadCount, clearConversation, remove, recall };