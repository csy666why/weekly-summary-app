...
const DEFAULT_SYSTEM = [
  ...
  "7. 按【上周工作总结】【收获体会】【下周期望】三个板块输出；上周工作总结按日期写具体做了什么，收获体会写真实感受，下周期望写下周想学/想做的事。",
...
function buildUser(mode, { notes, sectionsText, extra, target }) {
  ...
  const instructions = {
    generate: "请根据以上原始记录，帮我写一份完整的每周小结，包含【上周工作总结】【收获体会】【下周期望】三个板块。",
...
/* ---------- AI 生成图片 ---------- */
/* 中文→英文翻译（免费 MyMemory，用于 Pollinations 生图；失败则原样返回） */
const translationCache = new Map();
function hasCJK(text) { return /[\u4e00-\u9fff\u3400-\u4dbf]/.test(String(text || "")); }
async function translateToEnglish(text) {
  const key = String(text || "");
  if (translationCache.has(key)) return translationCache.get(key);
  try {
    const q = encodeURIComponent(key.slice(0, 500));
    const res = await fetch("https://api.mymemory.translated.net/get?q=" + q + "&langpair=zh-CN|en", { signal: AbortSignal.timeout(8000) });
    if (res.ok) {
      const j = await res.json();
      const t = j && j.responseData && j.responseData.translatedText;
      const out = (t && t.trim()) || key;
...
async function generateImage({ prompt, width, height, provider, apiKey, model }) {
  const w = width || 512;
  const h = height || 512;
  const p = String(prompt || "").trim();
  if (!p) throw new Error("图片描述不能为空");
  if (provider === "siliconflow") {
    const url = "https://api.siliconflow.cn/v1/images/generations";
    const res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": "Bearer " + (apiKey || "") },
      body: JSON.stringify({ model: model || "black-forest-labs/FLUX.1-schnell", prompt: p, image_size: w + "x" + h, batch_size: 1 })
    });
    if (!res.ok) throw new Error("生图服务返回 " + res.status);
    const j = await res.json();
    const item = j && j.images && j.images[0];
    if (!item) throw new Error("生图服务无返回");
    ...
    throw new Error("生图服务无返回");
  }
  // 默认：pollinations.ai（免费、无需 Key；中文提示词先翻译成英文）
  let finalPrompt = p;
  if (hasCJK(p)) finalPrompt = (await translateToEnglish(p)) + ", high quality, detailed, photorealistic";
  const url = "https://image.pollinations.ai/prompt/" + encodeURIComponent(finalPrompt) + "?width=" + w + "&height=" + h + "&nologo=true";
  const res = await fetch(url, { signal: AbortSignal.timeout(90000) });
  if (!res.ok) throw new Error("生图失败（" + res.status + "），请稍后重试");
  const buf = Buffer.from(await res.arrayBuffer());
  const ct = res.headers.get("content-type") || "image/jpeg";
  return { buffer: buf, mime: ct };
}

module.exports = { chat, buildSystem, buildUser, normalizeBaseURL, endpointFor, generateImage };
