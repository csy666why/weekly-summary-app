...
function requestsOf(spaceId) {
  ...
  return {
    ...
    rejected: all.filter((r) => r.from === spaceId && r.status === "rejected").sort((a, b) => (a.createdAt < b.createdAt ? 1 : -1)).slice(0, 10)
  };
}
function respondRequest(id, spaceId, accept, reason) {
  const all = loadRequests().requests;
  const r = all.find((x) => x.id === id && x.to === spaceId);
  if (!r) return null;
  r.status = accept ? "accepted" : "rejected";
  r.reason = accept ? "" : String(reason || "").slice(0, 100);
...
function reload() { cache = null; reqCache = null; }
module.exports = { reload, areFriends, friendsOf, addFriend, removeFriend, createRequest, requestsOf, respondRequest };